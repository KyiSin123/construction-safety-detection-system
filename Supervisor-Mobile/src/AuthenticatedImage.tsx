...
type Props = { path: string; token: string; style: StyleProp<ImageStyle>; resizeMode?: 'cover' | 'contain' };
const absoluteUrl = (path: string) => /^https?:\/\//i.test(path)
  ? path : `${API_BASE_URL}${path.startsWith('/') ? path : `/${path}`}`;

export function AuthenticatedImage({ path, token, style, resizeMode = 'cover' }: Props) {
  ...
  return <Image
    ...
    style={style} resizeMode={resizeMode} onError={() => setError('Image unavailable')}
  />;
}
