...
export type Violation = {
  ...
  identity_status: string; review_status: 'pending' | 'worker_submitted' | 'resolved' | 'ignored'; review_reason?: string;
  reviewed_by?: string; review_updated_at?: string; delivery_status: string; notified_at?: string;
  snapshot_count: number; alert_priority: 1 | 2 | 3; is_read: boolean; read_at?: string;
};
...
export type AttendanceRequest = { id:number; action:'check_in'|'check_out'; requested_at:string; reason:string; status:'pending'|'approved'|'rejected'; reviewer_name?:string; decision_reason?:string; decided_at?:string; created_at:string; worker_number:string; worker_name:string; worker_team?:string };
...
export const api = {
  ...
  attendanceRequests: (token:string,status:string) => request<AttendanceRequest[]>(`/api/mobile/attendance-requests?status=${encodeURIComponent(status)}`,token),
  decideAttendance: (token:string,id:number,decision:'approved'|'rejected',reason:string) => request<{message:string}>(`/api/mobile/attendance-requests/${id}/decision`,token,{method:'PATCH',body:JSON.stringify({decision,reason})}),
};
