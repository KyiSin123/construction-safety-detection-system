...
def seed(db):
    ...
    c.execute('''INSERT INTO workers(worker_number,name,team,password_hash,is_active,phone,email)
        VALUES(%s,%s,%s,%s,1,%s,%s) ON DUPLICATE KEY UPDATE name=VALUES(name),team=VALUES(team),
        password_hash=VALUES(password_hash),is_active=1,phone=VALUES(phone),email=VALUES(email)''',
        (WORKER_NUMBER,'Demo Worker','Demo Team',generate_password_hash(PASSWORD),'+95 9 123 456 789','demo.worker@example.com'))
    ...
    for days in (1,2,3,5,8):
        ...
        c.execute('''INSERT INTO attendance_records(worker_number,attendance_date,check_in_at,check_out_at,recorded_by)
            VALUES(%s,%s,%s,%s,'ID detection') ON DUPLICATE KEY UPDATE check_in_at=VALUES(check_in_at),
            check_out_at=VALUES(check_out_at),recorded_by=VALUES(recorded_by)''',(WORKER_NUMBER,day,check_in,check_out))
    ...
    request_rows=[
        ...
        ('check_out',now-timedelta(days=6,hours=-1),'Camera did not read my badge','approved',supervisor[0] if supervisor else None,'Demo Supervisor',None),
        ('check_in',now-timedelta(days=10,hours=8),'Badge reflection prevented detection','rejected',supervisor[0] if supervisor else None,'Demo Supervisor','Site log did not confirm arrival'),
    ]
    for action,requested,reason,status,reviewed_by,reviewer_name,decision_reason in request_rows:
        c.execute('''INSERT INTO attendance_requests(worker_number,action,requested_at,reason,status,
            reviewed_by,reviewer_name,decision_reason,decided_at)
            VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)''',(WORKER_NUMBER,action,requested,reason,status,
            reviewed_by,reviewer_name,decision_reason,now if status!='pending' else None))
    conn.commit(); c.close(); conn.close()


def cleanup(db):
    conn=db._connect(); c=conn.cursor()
    c.execute('DELETE FROM attendance_requests WHERE worker_number=%s',(WORKER_NUMBER,))
...
def main():
    parser=argparse.ArgumentParser(description=__doc__); parser.add_argument('--cleanup',action='store_true'); args=parser.parse_args()
    load_dotenv(os.path.join(os.path.dirname(__file__),'.env')); db=Database(); db.init_db()
    if args.cleanup:
        cleanup(db); print('Removed the demo worker and demo-owned history.')
    else:
        seed(db); print(f'Demo worker ready: {WORKER_NUMBER} / {PASSWORD}')
