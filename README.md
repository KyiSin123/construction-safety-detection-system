...
The root `.gitignore` keeps secrets, runtime records, generated mobile files, and
large machine-learning artifacts out of Git/cloud repositories. In particular,
do not commit:

- `.env` files, signing keys, or Firebase service configuration files
- `src/detections.db`, snapshots, worker proof photos, settings, or counters
- `node_modules`, Expo state, native builds, APK/AAB/IPA files
- training datasets, downloaded source media, generated runs, or model weights

Keep the provided `.env.example` files in Git and copy them to `.env` for local
configuration. A Firebase `google-services.json` used by Supervisor Mobile must
be downloaded or supplied privately and placed in `Supervisor-Mobile/`.

Ignored files that were committed previously remain in Git history until they
are explicitly removed from Git's index. Review that change carefully before
publishing because databases, snapshots, `.env` files, and service credentials
...
### **src/** - Production-Ready Web Application
...
- **`database.py`**
  - MySQL database interface for persistent data storage
  - Tables for:
...
- **MySQL database** - Stores detections, snapshots, alerts, and workers
...
### Web Application
The `src/` directory contains the production-ready Flask web application. The live detection operator UI is served at `/`; the separate administrator site is served at `/admin` and is used to manage workers, supervisor accounts, settings, and audit history. Supervisors receive and resolve alerts through the mobile application.
